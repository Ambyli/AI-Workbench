...
| `PORT_TRINO` | `8013` | Trino data lake: host port `trino-coordinator` binds to. Serves the web UI, HTTP API, and JDBC endpoint. LAN-only for now — auth is a follow-up. |
...
- **Catalogs are `.properties` files, not compose entries.** Adding a new data source (real S3, GCS, BigQuery, or another Postgres) is: drop a file in `ai/trino/catalogs/` + `docker compose restart trino-coordinator`. Do NOT edit the compose file for a new catalog — only Trino needs to know about it, and hot-reload of catalogs is trivial.
