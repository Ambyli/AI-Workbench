...
| `PORT_N8N_DB` | `5435` | n8n: host port `n8n-db` binds to for remote querying (psql / DBeaver / DataGrip). Lives in the `PORT REGISTRY` block at the top of `.env`, grouped with the other Postgres ports (`5432` litellm_db, `5433` roofix-db, `5434` sandbox-db, `5435` n8n-db). n8n itself has **no** host port — the editor is reached via `oauth2-proxy-n8n` (dedicated sidecar) at `https://n8n.zeoenergy.com/`. |
| `OAUTH2_PROXY_N8N_COOKIE_SECRET` / `_COOKIE_NAME` / `_COOKIE_DOMAINS` / `_COOKIE_SAMESITE` / `_REDIRECT_URL` / `_UPSTREAMS` | _(unset — required)_ | n8n: overrides for the dedicated `oauth2-proxy-n8n` sidecar in `ai/n8n/docker-compose.n8n.yml`. Everything not listed here (provider, Google client id/secret, group gate, SA-key, templates dir, cookie expire/refresh) is inherited from the openwebui-side `OAUTH2_PROXY_*` block above. The two instances share the Google OAuth client — just add `https://n8n.zeoenergy.com/oauth2/callback` to its authorized redirect URIs before first start. `_COOKIE_SECRET` must be distinct from `OAUTH2_PROXY_COOKIE_SECRET`. |
...
- **Deployed on its own subdomain (`n8n.zeoenergy.com`), NOT under `chat.zeoenergy.com/n8n/`.** n8n's built-in subpath routing (`N8N_PATH=/n8n/`) is broken upstream ([n8n-io/n8n#19635](https://github.com/n8n-io/n8n/issues/19635), [#18596](https://github.com/n8n-io/n8n/issues/18596)) as of 1.108.2 — static-asset requests hit the SPA fallback and return HTML instead of JS/CSS. We hit this on first deploy; subdomain sidesteps the bug entirely. Do NOT try to switch back to subpath without confirming the upstream fix has landed.
- **Dedicated `oauth2-proxy-n8n` sidecar lives in `ai/oauth2-proxy/docker-compose.oauth2-proxy.yml` alongside the openwebui-side instance** — all SSO gates in one file so the auth surface is inventoriable in one place, and `make up oauth2-proxy` brings up every gate together. The n8n stack (`make up n8n`) only owns `n8n` + `n8n-db`. Shared with the openwebui instance: Google OAuth client, access-group gate, service-account JSON, branded templates. Distinct: cookie name (`_oauth2_proxy_n8n`), cookie secret, redirect URL (`https://n8n.zeoenergy.com/oauth2/callback`), upstream (`http://n8n:5678`), SameSite (`lax`, not `none` — n8n is never iframed).
- **All container config lives in `.env`, not the compose files.** Both `ai/n8n/docker-compose.n8n.yml` and `ai/oauth2-proxy/docker-compose.oauth2-proxy.yml` are pure `${…}` templates — same discipline as `ai/openwebui/docker-compose.openwebui.yml`. Edit `.env` (the `## n8n subsystem` + `## oauth2-proxy-n8n` blocks), not the compose YAML.
...
- **First-start requires two manual steps outside the repo**: (1) add `https://n8n.zeoenergy.com/oauth2/callback` to the existing Google OAuth 2.0 client's authorized redirect URIs (same client openwebui uses — `OPENWEBUI_GOOGLE_CLIENT_ID`); (2) add a Cloudflare tunnel public hostname `n8n.zeoenergy.com` → HTTP → `oauth2-proxy-n8n:4180`. Both are documented in `ai/n8n/N8N.md § First-run checklist`.
