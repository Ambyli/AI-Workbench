...
#
# One-time install:
#   Bash / Git Bash / WSL:  eval "$$(make completion-bash)"  (add to ~/.bashrc)
#   PowerShell:             . "$(CURDIR)/scripts/completion.ps1"  (add to $$PROFILE)
#
# After install, `make up oa<Tab>` completes to `make up oauth2-proxy`, etc.
# ---------------------------------------------------------------------------

# Introspection targets the completion scripts call to stay in sync with the
# stack registry above. `make -s list-stacks` prints all stack names; `make -s
# list-services <stack>` prints the service list for that stack.
list-stacks:
	@echo $(STACKS)
...
_make_ai_complete() {
    local cur verb stack idx w verbs stack_verbs
    COMPREPLY=()
    cur="$${COMP_WORDS[COMP_CWORD]}"
    verbs="setup network up down clean very-clean build logs help completion completion-bash list-stacks list-services"
    stack_verbs=" up down clean very-clean build logs list-services "

    # Walk positional args (skip flags and VAR=VAL) to find the verb + stack
    ...
    case "$$stack_verbs" in
        *" $$verb "*)
            if [ -z "$$stack" ]; then
                COMPREPLY=( $$(compgen -W "$$(make -s list-stacks 2>/dev/null)" -- "$$cur") )
            else
                COMPREPLY=( $$(compgen -W "$$(make -s list-services $$stack 2>/dev/null)" -- "$$cur") )
            fi
            ;;
    esac
}
complete -F _make_ai_complete make gmake
endef
export COMPLETION_SCRIPT
...
completion:
	@echo ''
	@echo 'Tab completion for `make` verbs, stacks, and services.'
	@echo ''
	@echo 'Bash / Git Bash / WSL — one-shot (current shell only):'
	@echo '  eval "$$(make -C $(CURDIR) completion-bash)"'
	@echo ''
	@echo 'Bash / Git Bash / WSL — permanent (append to ~/.bashrc):'
	@echo "  echo 'eval \"\$$(make -C $(CURDIR) completion-bash)\"' >> ~/.bashrc"
	@echo ''
	@echo 'PowerShell — one-shot (current session only):'
	@echo "  . '$(CURDIR)/scripts/completion.ps1'"
	@echo ''
	@echo 'PowerShell — permanent (append to $$PROFILE):'
	@echo "  Add-Content \$$PROFILE \". '$(CURDIR)/scripts/completion.ps1'\""
	@echo ''

.PHONY: setup network up down clean very-clean build logs help completion completion-bash list-stacks list-services
