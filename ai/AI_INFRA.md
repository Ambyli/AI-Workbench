...
| [`docker-compose.roofix.yml`](docker-compose.roofix.yml) | `roofix-bridge` | _(internal only)_ | [ROOFIX.md](ROOFIX.md) |
...
- **Roofix bridge** — packaged in `docker-compose.roofix.yml`. Internal worker; does NOT receive inbound traffic. APScheduler ticks every `TICK_INTERVAL_SECONDS` (default 300s); each tick fetches unread Roofix mail via the Gmail MCP, decides per-event (rules first, LiteLLM fallback), and writes back via the Phoenix MCP. Ambiguous email events trigger a proposal fetch via `RoofixScraperClient` (`ai/roofix/bridge/components/roofix_scraper_client.py`), which POSTs to `interceptor-api`'s `/capture` under the `roofix` named profile. The old `roofix-scraper` service was retired — proposal captures now share the generic `interceptor-api` container with any other logged-in-site capture use case. Operators refresh the Roofix session by uploading a captured Chrome user-data-dir to `interceptor-api`'s `/profiles/roofix/refresh` (see [INTERCEPTOR_API.md](INTERCEPTOR_API.md)).
