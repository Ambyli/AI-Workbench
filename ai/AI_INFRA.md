...
| [`semantic-router/docker-compose.semantic-router.yml`](semantic-router/docker-compose.semantic-router.yml) | `vllm-sr-envoy`, `vllm-sr-router`, `vllm-sr-models-init` (one-shot), `vllm-sr-dashboard` (`dashboard` profile) | `8021` (envoy listener) — **only** published port; router `50051`/`8080`/`9190` and envoy admin `9901` never mapped. `8022` dashboard on `127.0.0.1` only | [SEMANTIC_ROUTER.md](semantic-router/SEMANTIC_ROUTER.md) |
...
flowchart TB
    ...
    subgraph SRG["semantic-router/docker-compose.semantic-router.yml"]
        SRE["vllm-sr-envoy<br/>:8021 -> :8899 listener<br/>ext_proc + Lua bearer auth"]:::svc
        SRR["vllm-sr-router<br/>internal :50051 / :8080 / :9190<br/>signals -> decision -> looper"]:::svc
    end
    ...
    SRR  ==>|"candidate sub-requests<br/>(back through SRE)"| LL
    SRR  -. Vela bundles .-> HF
...
- **The semantic router is the one place LiteLLM calls itself** — every other edge out of `LL` terminates at a model server. `LL ==> SRE` is the alias `auto` (`openai/vllm-sr/auto`, `api_base: http://vllm-sr-envoy:8899/v1`); `SRE --> SRR` is the ext_proc gRPC hop on `:50051`; and `SRR ==> LL` is the router issuing one sub-request **per candidate** back into LiteLLM, each steered by an `x-selected-model` header that Envoy matches to a per-model cluster — all of which resolve to `litellm:4000`. So a single `auto` turn can be one LiteLLM request (confidence, first rung answers) or seven (remom `breadth_schedule: [3, 2]` plus synthesis). **The loop is the design, and the guard is two-sided**: the router's `providers.models` lists only concrete aliases and never `auto` or any `vllm-sr/*` name, and `SEMANTIC_ROUTER_LITELLM_KEY` is a LiteLLM virtual key scoped to those six aliases and nothing else — so even a mis-edited `config.yaml` 401s instead of spinning until the 1800 s listener timeout. Only Envoy's listener is published (`8021`); the router's `50051` / `8080` / `9190` and Envoy's admin `9901` are never mapped, the last because it serves `/quitquitquit` and a config dump containing the listener bearer token. Envoy runs ext_proc with **buffered** request and response bodies, so nothing reaches the caller until the whole panel finishes — hence `stream_timeout: 1800` on the alias and a chat that shows a waiting state rather than tokens. `SRR -. Vela bundles .-> HF` is the router fetching its own classifier weights on first start (the pinned CLI has no download subcommand; the router image does it). See [SEMANTIC_ROUTER.md](semantic-router/SEMANTIC_ROUTER.md).
...
| vllm-sr-envoy (semantic router listener) | `8021` |
| vllm-sr-dashboard | `8022` _(bound to `127.0.0.1` only, behind the `dashboard` compose profile)_ |
