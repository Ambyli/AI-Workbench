...
FOUND = lambda pid=101: {"found": True, "project_id": pid, "ambiguous": False}
...
CASES = [
    ...
    (
        "hic_executed",
        {"found": True, "ambiguous": True, "candidate_count": 3},
        "escalate",
        True,
    ),
]
...
def run():
    ...
    for label, ctx, want_action, want_human in CASES:
        ...
        ctx_desc = (
            ...
            else "ambiguous" if ctx.get("ambiguous") else "not-found"
        )
        if ok:
            passed += 1
            ...
            failed += 1
            print(
                f"FAIL  {label:26s} [{ctx_desc:9s}] -> got {d['action']}/"
