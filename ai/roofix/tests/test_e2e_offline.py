...
class MockPhoenix:
    ...
    def update_chatter(self, project_id, note_text):
        ...
        return Result(ok=True, detail="mocked", dry_run=True,
    ...
    def ensure_entity_and_project(self, extracted):
        ...
        return Result(
            ok=True, detail="mocked create", dry_run=True,
            data={
                "entity_id": 9999,
                "phoenix_project_id": 8888,
                "created_entity": True,
...
def run() -> bool:
    ...
    def _test_estimate_parsed():
        ...
        assert d["project_id"] is None, d  # tokenized URL only
    ...
    def _test_batch_estimate():
        ...
        decisions = process_batch(
            ...
            phoenix=phoenix, log=audit, milestone_map=None)
    ...
    def _test_run_estimate():
        ...
        decisions = orchestrator_run(
            listener=listener, phoenix=phoenix,
            milestone_map=None, log=audit)

        assert len(decisions) == 1, f"expected 1 decision, got {len(decisions)}"
        d = decisions[0]
        assert d["action"] == "ignore", d
        assert d["source"] == "rule", d
        # Phoenix IS called for identity resolution (project_id=None in estimate).
        identity_calls = [c for c in phoenix.calls if c[0] == "find_project_by_identity"]
        assert len(identity_calls) == 1, f"expected 1 identity call, got {len(identity_calls)}"
    ...
    def _test_run_multiple_estimates():
        ...
        for d in decisions:
            assert d["action"] == "ignore", d
            assert d["source"] == "rule", d
        # Two identity lookups (one per estimate, since project_id=None).
        identity_calls = [c for c in phoenix.calls if c[0] == "find_project_by_identity"]
        assert len(identity_calls) == 2, f"expected 2 identity calls, got {len(identity_calls)}"
