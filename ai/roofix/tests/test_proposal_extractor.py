...
def _test_unaccepted_extraction() -> list[str]:
    ...
    checks = {
        ...
        "roofix_id": "1784846253605x685099068151907800",
        "external_ref": "21GGPT",
...
def _test_missing_mget_docs() -> list[str]:
    ...
    if "mget_docs" not in (result.error or ""):
        return [
            f"empty mget_docs: error should mention mget_docs, got {result.error!r}"
        ]
    return []
...
def _test_homeowner_only_no_order1() -> list[str]:
    ...
    (homeowner is enough to skip the hard-fail branch) but roofix_id
    ...
    if result.roofix_id is not None:
        problems.append(
            f"homeowner-only roofix_id: expected None (URL is not a source), "
            f"got {result.roofix_id!r}"
        )
    if result.first_name != "Jane":
        problems.append(f"homeowner-only first_name: got {result.first_name!r}")
    if result.is_accepted:
        problems.append(
            "homeowner-only + opportunity stage: is_accepted should be False"
        )
    return problems
...
_CASES = [
    ...
    (
        "identity: homeowner only, no order1 -> roofix_id=None",
        _test_homeowner_only_no_order1,
    ),
    ("acceptance: hic executed alone", _test_hic_executed_alone_is_accepted),
    (
        "acceptance: hic draft alone is NOT accepted",
        _test_hic_present_but_not_executed_is_not_accepted,
    ),
