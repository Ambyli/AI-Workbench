...
The guard has two layers. A cheap metadata **lookup** (`_find_active_session_by_chat`) short-circuits the common "created twice, seconds apart" case before a slot is ever taken, so a prevented duplicate is free. Underneath it, a partial **UNIQUE** index `jobs_chat_id_active_uniq` — over the `spawning`/`starting`/`running` phases — makes the row INSERT itself the point of mutual exclusion: two `create`s for one chat firing within the same millisecond both pass the lookup, then one INSERT wins and the other raises `UniqueViolationError`, which `_do_create` / `_reuse_or_spawn` catch and turn into "return the winner." So a chat can **never** double-spawn, even under a dead-heat race — the database, not a time window, is the arbiter. (Rows with no `chat_id` index a NULL expression, and NULLs are distinct in a UNIQUE index, so unscoped sandboxes never collide.) `chat_id` is stored on the job's metadata and replayed across self-heal respawns, so the chat→container binding survives a reaped-and-recreated container. Omit `chat_id` to keep the historical always-spawn behavior.
