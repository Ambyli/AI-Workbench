...
Public API
----------
open_settings_dialog()   — open the window (blocks until closed)
"""
...
def open_settings_dialog() -> None:
    ...
    # ── Title bar (identical to main popup) ───────────────────────────────────
    ...
    def _save():
        ...
        for key, _label, wtype, _hint in _FIELDS:
            if key == "__head__" or key not in entries:
                continue
            var = entries[key]
            if wtype == "bool":
                updates[key] = var.get()
        ...
        if errors:
            status_var.set("Error: " + errors[0])
            return

        config.apply_updates(updates)

        # Update log handler levels live
        try:
        ...
        def _fade(step: int = 0, total: int = 15) -> None:
            ...
            status_lbl.config(fg=f"#{r:02x}{g:02x}{b:02x}")
            win.after(30, lambda: _fade(step + 1, total))

        win.after(1200, _fade)
    ...
    # ── Size and position ─────────────────────────────────────────────────────
    # Large temp geometry lets content compute its natural height before we constrain it
